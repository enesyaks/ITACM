...
router.get('/', asyncHandler(async (req, res) => {
  res.json({ success: true, data: await providerService.listContracts(req.query) });
}));

/** POST /api/contracts */
router.post('/', write, asyncHandler(async (req, res) => {
  res.status(201).json({ success: true, data: await providerService.createContract(req.body) });
}));
...
router.post('/:id/documents', write, express.json({ limit: '12mb' }), asyncHandler(async (req, res) => {
  ...
  const contract = await providerService.getContract(req.params.id);
...
router.get('/:id', asyncHandler(async (req, res) => {
  res.json({ success: true, data: await providerService.getContract(req.params.id) });
}));

/** PATCH /api/contracts/:id */
router.patch('/:id', write, asyncHandler(async (req, res) => {
  res.json({ success: true, data: await providerService.updateContract(req.params.id, req.body) });
}));

/** DELETE /api/contracts/:id */
router.delete('/:id', write, asyncHandler(async (req, res) => {
  res.json({ success: true, data: await providerService.deleteContract(req.params.id) });
}));
